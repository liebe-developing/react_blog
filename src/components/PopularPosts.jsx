...
const PopularPosts = () => {
  return (
    ...
    >
      ...
      <Flex
        className="sm:items-center max-sm:flex-col justify-center items-center w-full gap-2"
        maxH="580px"
      >
        <Box>
          {RANDOM_POSTS.slice(0, 1).map((post, idx) => (
            <Box key={idx} pos="relative" className="group rounded-xl" overflow="hidden">
              <Image
                src={post.img}
                alt={post.title}
        ...
        <Box
          className="flex flex-col justify-center items-center px-5 gap-5"
          maxH="550px"
        >
          {RANDOM_POSTS.slice(0, 3).map((post, idx) => (
            <Flex key={idx} gap={4} className="items-center max-sm:flex-col">
              <Image
                ...
                alt={post.title}
                rounded="xl"
                w={{ base: "350px", md: "180px" }}
                h={{ base: "350px", md: "180px" }}
              />
              <Flex className="flex-col justify-between h-full gap-4">
                <Badge
                  variant="solid"
                  colorScheme="green"
                  rounded="xl"
